...
export const generateNewPdfFromOpenAi = async (originalPdfUrl: string, text: string, url: string) => {
  ...
  const generatedPdf = await db.file.create({
    data: {
      ...
      userId: user.id, // Ensure you associate the PDF with the correct user
    },
  });
